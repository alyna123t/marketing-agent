...
*Author: Alyna · 2026-07-29 · For: Adrian*
...
executed. The knobs you can turn are at the bottom.
...
**The bet:** a shiplog that's only ships flatlines — Aeon posts pure ship-as-post to
150.1K followers and reaches 1.4–5% of them. Ours leads with the week's real result and
lets the ships answer it. Losses stay in.

> Down $2,880 this week across 41 agents, 47% win rate (168 wins / 190 losses). Almost all of it is one strategy: News Chaser lost $3,970 buying a Fed-surprise rate market at 0.64 on a headline that later got corrected, then rode it down to 0.31. Six agents, one source, no check.
>
> **We shipped the check (#412).** News Chaser agents now need a second independent source confirming the same claim within 90 seconds before they can enter — no match, no entry, and the reason gets written to the agent's log. Backtested against the 14 corrected-headline events in our logs since May: it blocks 11 of them, misses 3 where every source ran the same wire copy, and would have blocked 2 entries that were actually fine. It's behind the `HEADLINE_GATE` flag, off today, default-on 2026-07-31.
>
> Also merged:
...
number has an evidence-map entry. On a quiet week this format outputs "no post — nothing
cleared the gate" instead — tested.*
...
> **What changed.** News Chaser used to let an agent enter on a single headline claim. As of PR #412 (adrian, merged 2026-07-24, closes SIM-231 "Agents enter on unverified headlines"), a headline claim needs a second independent source to agree before entry is allowed.
>
...
## The knobs (your call — defaults in bold)

1. **Cadence** — **shiplog + digest weekly (Mon run)**, feature-built on demand when a
   ship earns it. Alternative: digest mid-week, closer to when tweets are fresh.
2. **Does the shiplog carry the PnL beat?** — **Yes** (the Aeon flatline is the argument).
   Alternative: ships-only, PnL stays in a separate receipts-style post.
3. **Length** — these are on the long end of the range. Each format cuts cleanly (digest
   to 3 items, shiplog to anchor + 2).
4. **Tier defaults** — **Tier 2 (Alyna, <24h) is the norm**; Tier 3 (you) only when the
   post itself creates new exposure — e.g. the venue rumor above. Too loose/tight?
5. **Voice** — per your pivot note, the agent writes plain and *doesn't* style-match;
   voice is applied by whoever pastes. Confirm that's still the intent.
6. **Which feed surface** (TG? Discord?) and **who pastes** — not encoded anywhere yet;
   also note we borrowed the formats from Aeon's *Telegram* feed but have only audited
   their X account (`x-account-audit.md` §4).

**What happens after your pass:** you mark up this doc (or the posts inline), format
changes go into the skill files, and the first live week runs on real intake
(`shared-knowledge/marketing/intake/README.md` — v1 is a weekly human paste; the
GitHub Actions runner is `.github/workflows/marketing-agent.yml`, needs only an
